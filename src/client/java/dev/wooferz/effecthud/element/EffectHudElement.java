...
import static dev.wooferz.effecthud.StatusEffectHUDClient.LOGGER;
public class EffectHudElement extends HUDElement {

    public EffectHudConfig config = new EffectHudConfig();

    public EffectHudElement() {
        super("Status Effect HUD", 10, 0, 120, 40, 1, StatusEffectHUD.MOD_ID, "effect-hud",  HudAnchor.HorizontalAnchor.LEFT, HudAnchor.VerticalAnchor.MIDDLE);
    }
    ...
    public void render(int x, int y, int width, int height, DrawContext drawContext, float v) {
        ...
        if (player != null) {
            ...
            for (StatusEffectInstance effect : effects) {
                ...
                //LOGGER.info(String.valueOf(texture));
                //Identifier.ofVanilla("textures/"+effectTypeKey.registry.path}/${effectTypeKey.value.path}.png")

                drawContext.drawText(mc.textRenderer, effectType.getName(), 2+x+19, 3+2+y+i*25, color, true);
